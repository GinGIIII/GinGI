...
Date Date::operator - (const Date& d) {
    ...
    if (isOurEra == d.isOurEra) {
        ...
        result.day -= d.day;
        if (!result.isOurEra && result.day < 0) {
            dayInM[2] += 1;
            result.month -= 1;
...
int main() {
    Date d1(2001, 3, 0, 0, 0, 0, 1);
    Date d2(2000, 3, 0, 0, 0, 0, 1);
    Date d3(4000, 3, 0, 0, 0, 0, 1);
    std::cout << d1 + d2 - d3 << std::endl; //
    std::cout << d1.add(1998, 0, 0, 0, 0,0, 1).subtract(4000, 12, 0, 0, 0, 0, 1) << std::endl;//
